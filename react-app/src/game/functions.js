import { music, highScores, gameProps, remainingBullets, spaceBody, rows, columns, border, playerShip, enemyShip, enemyBullet, playerBullet, bothBullets, emptyCell, fillerCell, sounds } from './vars.js';
...
export function sortAndSliceScores() {
    highScores = highScores.sort( (a,b) => b.score - a.score).slice(0,10);
}

export function registerKeyPressed(event) {
    if (isPlayable()) {
        movePlayer(event.key);
    }
    
}
export function presswButton() {
    if (isPlayable()) {
        movePlayer('w');
    }
}

export function pressaButton() {
    if (isPlayable()) {
        movePlayer('a');
    }
}

export function pressdButton() {
    if (isPlayable()) {
        movePlayer('d');
    }
}
...
export function renderHighScore(highscore, table) {
...
export function renderInitialHighScores() {
    recoverScores();
    refreshHighScores();
}
...
export function getNextCell(direction, row, col) {
    switch (direction) {
        case 'above':
...
export function moveCell(origin, destiny) {
...
export function killEnemy(cell) {
    clearCell(cell);
...
export function hitPlayer(cell) {
    gameProps.lives--;
